...
## IT IS AN UPDATED VERSION OF PSAPROB, TO ALLOW FOR INSTANCES WHEN NONE OF THE COMPUTATIONAL 
## DIFFICULTIES MENTIONED IN README.TXT OCCUR
...
PSAprob2<-function(ntrans=3, ncovs=c(1,1,2), 
...
if (is.na(dist2[i])==FALSE) H[[i]]<-rbind(H[[i]], H_ext[[i]])
  ...
  tt<-c(timeseq,timeseq_ext)
    ##### create the cumulative hazard matrix continued
      ...
      if (is.null(Hazerror[[1]]==FALSE)) Hazerror<-as.numeric(Hazerror)
  

      
      ### adjust hazards that had differences of > 1 in consecutive hazards
      if (is.null(Hazerror[[1]]==FALSE)){
      for (k in 1:nruns){
